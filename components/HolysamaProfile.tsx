...
import {HolysamaCare} from "@/components/PartnerProfileDetails";
...
export function HolysamaProfile({locale, clinic}: {locale: string; clinic: Clinic}) {
  ...
  const detail = (partnerDetails[locale as keyof typeof partnerDetails] ?? partnerDetails.de).julia;
  return <>
    ...
    <section className="section"><div className="container holysama-about"><SectionHeading eyebrow={c.aboutLabel} title={c.aboutTitle} /><p className="lead">{c.about}</p><p className="lead partner-paragraph">{detail.career}</p></div></section>
    <HolysamaCare locale={locale} />
    <section className="section section-soft"><div className="container holysama-education"><Image src="/clinics/holysama/movement.webp" alt={detail.movementAlt} width={966} height={757} sizes="(max-width:767px) 90vw, 40vw" /><div><span className="eyebrow">{c.networkLabel}</span><h2 className="section-title">{c.networkTitle}</h2><p className="lead">{c.networkBody}</p><a className="button button-secondary" href="https://holysama.de/wp-content/uploads/2023/05/Holysama_Behandlungsablauf.pdf" target="_blank" rel="noopener noreferrer">{c.process}<ArrowUpRight size={16} aria-hidden="true" /></a><p className="health-point-note">{c.note}</p></div></div></section>
